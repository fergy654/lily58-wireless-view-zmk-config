...
//     lv_canvas_draw_img(canvas, 34, 1, &bolt, &img_dsc);
//   }
}
...
//     lv_canvas_draw_img(canvas, 34, 21, &bolt, &img_dsc);
//   }
}

static void draw_central_status(lv_obj_t *canvas, const struct status_state *state) {
...
static void set_peripheral_battery_status(struct zmk_widget_status *widget, struct zmk_peripheral_battery_state_changed state) {
...
static int peripheral_battery_status_listener(const zmk_event_t *eh) {
    ...
    struct zmk_widget_status *widget;
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) {
        set_peripheral_battery_status(widget, *ev);
    }

    return ZMK_EV_EVENT_BUBBLE;
}

ZMK_LISTENER(peripheral_battery_status, peripheral_battery_status_listener);
